...
struct document get_document(char* text) {
    ...
    for(int i=0;i<strlen(text);i++){
        if(text[i] == ' '){
            ...
            c=0;   //Start the characters of the new word from zero.
            w++;   //New word is formed.
            continue; //Go back to the start of loop
        }
        if(text[i] == '.'){
            doc.data[p].data[s].data[w].data[c] = '\0';
            doc.data[p].sentence_count++;
            doc.data[p].data[s].word_count++; //Fullstop also creates a new word.
            c=0;
            w=0;   //Back to word 1 of the new sentence.
            s++;   //New sentence is formed.
            continue;
        }
        if(text[i] == '\n'){
            doc.paragraph_count++;
            c=0;
            w=0;
            s=0;   //Back to sentence 1 of the new paragraph
            p++;   //New paragraph is formed
            continue;
        }
